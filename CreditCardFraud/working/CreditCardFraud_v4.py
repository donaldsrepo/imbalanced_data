...
def custom_loss(y_true, y_pred):
    ...
    return loss
#clf.compile(optimizer='adam', loss=custom_loss, metrics=['accuracy']) 
...
a='''
metrics = Metrics()
clf.fit(X_train_arr, y_train_arr, epochs=200, verbose=1, 
        sample_weight=np.where(y_train_arr == 1,1.0,1.0).flatten(), 
        shuffle=True, callbacks=[metrics], validation_data=(X_test_arr,y_test_arr))'''
...
clf.compile(optimizer=opt, loss=custom_loss, metrics=['accuracy'])
...
clf.summary()
#clf.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy','auc','precision','recall'])
#clf.compile(optimizer='adam', loss='binary_crossentropy', metrics=[keras.metrics.BinaryAccuracy(name='accuracy'), keras.metrics.AUC(name='auc')])
...
#weight_for_1 = 0.01 * weight_for_1 
